...
- **The ballot theorem and conjecture** (`chernpp/sectors.py`). On the plane sector $\max M \le 1$,
  ...
  hyperplane lemma), via a bijection with monotone factorisations of a long cycle. *Conjecture*: $C(M) \ge b(M)$ for every $M$, equality exactly on the plane sector; this
