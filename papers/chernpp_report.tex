...
\subsection{The cost of \texorpdfstring{$\Qd_8$}{Q\_8}}

Between $d = 7$ and $d = 8$ the ambient dimension goes $34 \to 50$, $\deg\Qd_d$ goes $13 \to 22$,
the basic equations $21 \to 39$ and the defect-zero variables $12 \to 16$. The first saturation
step, $0.5$\,s at $d = 7$, does not finish in an hour.

\textbf{Question.} Is the obstruction the equations or the method? [BS, Remark 7.2] note that
standard algorithms exist for the equivariant dual of a \emph{toric} orbit but that none is known
for Borel orbits. The toric route was tried here and closed off: $\mathrm{eP}[T_d]$ computes in
$0.13$\,s even at $d = 7$ and matches the published codimension table, but does not divide $\Qd_6$,
so it is not a factor of the Borel class.
