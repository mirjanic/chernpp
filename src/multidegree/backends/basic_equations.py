...
def minimal_transversals(supports, bound):
    """
    Minimal primes of a monomial ideal, as index sets, up to codimension ``bound``.

    The minimal primes of a monomial ideal are generated by variables: ``P_S``
    contains the ideal exactly when ``S`` meets the support of every generator.
    So they are the minimal *transversals* of the supports, and finding them is
    combinatorics rather than primary decomposition -- which is what Sage's
    generic ``minimal_associated_primes`` would run, at a cost of about 40 s at
    ``d = 7`` against a few tenths of a second here.
    ...
    itself of size ``<= bound``, so nothing of interest is pruned; and it keeps
    the codimension check honest, since a component of codimension below
    ``bound`` would still be found.
    """
    antichain = {frozenset()}
        ...
        for partial in antichain:
            ...
            for index in support:
                ...
                if len(extended) <= bound:
                    grown.add(extended)
        # Keep only the minimal elements; a transversal containing another is
        # never minimal.
        by_size = sorted(grown, key=len)
        antichain = set()
        for candidate in by_size:
            if not any(kept <= candidate for kept in antichain):
                antichain.add(candidate)
    return sorted(antichain, key=lambda s: (len(s), sorted(s)))


def _multiplicity(exponents, indices, base_field, _cache=None):
    """
    ...
    repeat heavily across components, so they are cached.
    """
    keep = tuple(indices)
    restricted = set()
    for monomial in exponents:
        image = tuple(monomial[i] for i in keep)
        if any(image):
            restricted.add(image)
    # Drop anything divisible by another generator; Singular would do it anyway.
    minimal = tuple(
        sorted(
            m
            for m in restricted
            if not any(
                other != m and all(a <= b for a, b in zip(other, m)) for other in restricted
            )
        )
    )

    if _cache is None:
        _cache = _MULTIPLICITY_CACHE
    if minimal in _cache:
        return _cache[minimal]

    local_ring = PolynomialRing(base_field, len(keep), "y")
    ...
    value = (
        ...
        if hasattr(ideal, "vector_space_dimension")
        else ideal.gen().degree()
    )
    _cache[minimal] = value
    return value


#: Restricted ideals repeat across components; keyed by the minimal generators.
_MULTIPLICITY_CACHE = {}
...
class BasicEquationsBackend(MultidegreeBackend):
    ...
    def compute(self, family, order, base_field):
        ...
        logger.info(
            "A_%d: %d top-dimensional components of codimension %d",
            order,
            len(components),
            codimension,
        )
        ...
        for component in components:
            ...
            multiplicity = _multiplicity(exponents, indices, base_field)
