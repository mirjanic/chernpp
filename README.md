...
- **The ballot theorem and conjecture** (`chernpp/sectors.py`). On the plane sector $\max M \le 1$,
  ...
  hyperplane lemma), via a bijection with monotone factorisations of a long cycle. *Conjecture*: $C(M) \ge b(M)$ for every $M$ (proved for $d \le 5$: `chernpp/ballot.py`, exact isl inclusions; $d = 5$ via a residue-null gauge, a contour-free swap lemma and a dominant-cell bound), equality exactly on the plane sector; this
