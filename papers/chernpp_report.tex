...
at $d = 7$ its first step alone fails to finish.  Back-to-front, all twelve complete in $23$\,s.

Third, the two phases after the basis are combinatorics, not algebra.  The initial ideal is
...
\emph{transversals} of those supports, which a bounded incremental search finds in a few tenths of
a second where a general-purpose primary decomposition takes $41$\,s.  Bounding the search at the
expected codimension is sound --- every subset of a small transversal is itself small --- and
leaves the codimension check intact in both directions.  Multiplicities localise the same monomial
data, so restricting exponent vectors into one reused ring replaces $621$ fresh polynomial rings
and some $177{,}000$ substitutions in $34$ variables.

Over $\mathbb{Q}$ the total falls from $138$\,s to $85$\,s: saturation $63$\,s, final basis
$0.6$\,s, minimal primes $41 \to 4$\,s, multidegree $33 \to 17$\,s.  The multidegrees are identical
to the previous route at $d = 4, 5, 6$, and the $d = 7$ artifact is unchanged.  Nothing about the
mathematics changed.
