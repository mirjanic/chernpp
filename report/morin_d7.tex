...
Table~\ref{tab:rho} & $\rho$ on the registry; $\rho - \operatorname{corank}$ reaches $2$ & verified \\
\S\ref{sec:data} & $\Tp_{A_7}$ ($\ell \le 5$), $\Tp_{A_6}$ ($\ell \le 8$) exact and positive, $\min C = 1$ & verified \\
\ref{obs:kreweras} & plane sector: $C(M)$ = Kreweras number, $d \le 7$ & verified \\
...
Table~\ref{tab:rho} gives $\rho$ for the $87$ tables decided so far, out of $128$ non-Morin
tables in the registry. Most of the rest are of codimension $19$ to $26$ and exceeded the time
budget. In three of them ($I_{2,5}$, $\mathrm{III}_{2,6}$ and $\mathrm{III}_{3,5}$ at $\ell = 2$)
the LP reports $\Tp \notin C_2$, but its Farkas vector did not rationalise to a certificate that
verifies, so they are left undecided rather than reported. Each entry is decided by a linear
...
\item $\rho - s$ reaches $2$: $B_{5,3}$ at $\ell = 1$ has $s = 2$ and $\rho = 4$. $C_6$ and $C_{5,3}$
  have $s = 3$ and $\rho = 4$.
\end{itemize}
...
\begin{observation}[verified, $d \le 7$]\label{obs:kreweras}
On the plane sector $\max M \le 1$, write $M = (1^{d-b}, 1-s_1, \dots, 1-s_b)$ with $s_i \ge 1$.
Then
\[
  C(M) = \frac{d!}{(d-b+1)!\,\prod_i m_i!},
\]
the number of noncrossing partitions of $[d]$ with block sizes $s_i$ (Kreweras), where $m_i$ is
the number of blocks of size $i$. This holds for every plane-sector packet at $d \le 7$. The
external summary~\cite{FS} proves positivity on this sector; the observation identifies the
values. In particular $C(M) = 1$ on the sector exactly at $M = 0$ and $M = (1^{d-1}, 1-d)$, and
