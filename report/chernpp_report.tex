...
The multidegree numerator $\Qd_d$ is not the unique polynomial that correctly computes the Thom polynomial $\Tp_{A_d}$. We can perturb $\Qd_d$ by adding any ``residue-null'' kernel $P$ without altering the geometric results at any depth. Because strong positivity is false for the canonical B\'erczi--Szenes numerator, resolving the remaining negative coefficients requires exploiting this gauge freedom to find an alternative, positive representative. The idea, and the $d=5$ kernel that first showed it works, are due to the external reports \cite{FS,A6}; both are unrefereed.

\subsection{The kernel basis at \texorpdfstring{$d = 5$}{d = 5}}

At $d = 5$, enforcing exact nullity across the available Chern packets produces $6$ structural kernels ($K_i$). They span a $6$-dimensional subspace of the residue-null gauge space. Whether they span all of it is open: the null space of the packet map, which contains every residue-null kernel, has dimension $8$ at the depths tested. Each kernel represents a structural symmetry in the residue integral:
\begin{table}[ht]
\centering
\caption{The 6 structural kernels for $d=5$. They are not known to span the residue-null gauge space.}
...
\]
Because $K_4, K_5,$ and $K_6$ natively share the factor $(z_1 + z_4 - z_5)$, their linear combination $GBC$ neatly factors as well. Subtracting $GBC$ removes every negative coefficient of the baseline chamber series at $d=5$ on the tested range. 
...
in \cite{FS}; $P_5$ is a minimality refinement sitting on top of a settled result.
...
\bibitem{FS} Rim\'anyi positivity: findings summary (29 July 2026), unrefereed report, \texttt{papers/rimanyi\_positivity\_findings\_summary.pdf}.
\bibitem{A6} $A_6$ status report (31 July 2026), unrefereed, \texttt{papers/a6\_status.pdf}.
\end{thebibliography}
