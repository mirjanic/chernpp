...
\begin{table}[ht]
\centering
\caption{The top 10 most influential symmetry kernels identified by the optimal fractional gauge for $d=6$.}
\label{tab:d6_kernels}
\small
\begin{tabularx}{\textwidth}{@{}rLcc@{}}
\toprule
 Weight & Geometric Structure & Terms & Max $|c|$ \\
\midrule
 $-0.3488$ & symmetry $s_{5,6}$ absorbing 3 factors & 18 & 2 \\
 $-0.3451$ & symmetry $s_{1,2}$ absorbing 5 factors & 58 & 4 \\
 $-0.3365$ & partial abs.\ $s_{4,6}$ dropping $(-z_6 + z_3 + z_2)$ & 76 & 8 \\
 $-0.2815$ & symmetry $s_{2,3}$ absorbing 4 factors & 30 & 4 \\
 $-0.2645$ & symmetry $s_{1,2}$ absorbing 5 factors & 58 & 4 \\
 $0.2438$ & symmetry $s_{2,3}$ absorbing 4 factors & 30 & 4 \\
 $0.2223$ & symmetry $s_{1,2}$ absorbing 5 factors & 58 & 4 \\
 $0.2057$ & partial abs.\ $s_{4,6}$ dropping $(-z_6 + z_3 + z_2)$ & 76 & 8 \\
 $0.1873$ & symmetry $s_{2,3}$ absorbing 4 factors & 30 & 4 \\
 $0.1672$ & symmetry $s_{2,3}$ absorbing 4 factors & 30 & 4 \\
\bottomrule
\end{tabularx}
\end{table}

\begin{proposition}
At depth 45, this explicitly-constructed fractional gauge eliminates every negative coefficient in the chamber series. However, analytically verifying the same exact gauge at depth 46 reveals that 20 negative coefficients reappear. Thus, while the geometric construction successfully suppresses initial negativity, it is mathematically insufficient to certify asymptotic positivity at $d=6$.
\end{proposition}
...
\item Since \S\ref{sec:gauge} shows the numerator is free up to a residue-null kernel,
  ...
  nonnegative on its own. At $d = 5$ one is, and no certificate is needed. At $d = 6$ the
  construction gets close but does not close: it removes every negative coefficient up to a
  finite depth and then fails, so it is evidence rather than a proof.
\item The continuous LP relaxation is the correct algorithmic vehicle for exploring deep 
  ...
  Future work at $d \ge 7$ should extract exact rational boundaries from the continuous
  formulation rather than fight integrality. Note what this does and does not deliver: a
  feasible point at depth $T$ is a finite verification, and only an \emph{infeasible} LP at
  some $T$ settles anything at every degree, since truncation is an exact projection of the
  feasible set.
