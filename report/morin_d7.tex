...
{\footnotesize $^*$ given Theorem 5.4 of the unrefereed external summary~\cite{FS}.}
\end{center}
...
\begin{theorem}[plane sector; proved, given {\cite[Thm.~5.4]{FS}}]\label{thm:plane}
...
\begin{proof}
Put $n = d+1$. The external summary~\cite[Thm.~5.4]{FS} identifies $C(M)$, on this sector, with the
...
plane sector. That is $54{,}390$ packets for $d = 2, \dots, 7$, including every packet of charge $\le 6$ at $d = 7$ and all of $\Tp_{A_7}$ for
$\ell \le 5$ and $\Tp_{A_6}$ for $\ell \le 10$ (\texttt{results/ballot\_conjecture.json}). Off the
...
In the external summary's geometric model, $C(M) = \int_{C_{n,r}} m_\nu(E)$ over the closure of
